//-------------------------------------------------------------------------------------
package codex.xbit.api.server.trader.env.single.level5.signals.helpers.whalealert.crawler;
//-------------------------------------------------------------------------------------
import java.io.*;
import java.util.*;
...
import codex.xbit.api.server.trader.env.single.level5.signals.helpers.whalealert.crawler.analyzers.*;
...
//-------------------------------------------------------------------------------------
	public WhaleAlertSignal getSignal(CurrencyPair _currencyPair, Map<String, String> _arg){
		...
		for (int i = 0; i < _signalEvents.size(); i++) {
			...
			if(_signalEvent != null){
				...
				for (int j = 0; j < currencyPairs.size(); j++) {
					...
					getSignal(_currencyPair, null).onNext(_signalEvent);
				}
			}			
		}
	}
